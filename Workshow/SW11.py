...
import threading
import queue
import requests  # 用於載入真實數據

class MLBPlayerDatabase:
    def __init__(self):
        # 預設載入示例數據，防止 players_data 為空
    ...
    def load_real_data(self):
        """從 MLB Stats API 載入真實數據"""
        try:
            # 獲取打擊數據
            hitting_url = "https://statsapi.mlb.com/api/v1/stats?stats=season&group=hitting&season=2025&playerPool=all&limit=500"
            response = requests.get(hitting_url, timeout=10)
            response.raise_for_status()
            hitting_data = response.json()

            # 獲取投球數據
            pitching_url = "https://statsapi.mlb.com/api/v1/stats?stats=season&group=pitching&season=2025&playerPool=all&limit=500"
            response = requests.get(pitching_url, timeout=10)
            response.raise_for_status()
            pitching_data = response.json()

            # 合併數據（簡化處理，實際需根據 API 結構調整）
            players = []
            # 假設從 API 解析（這裡簡化為示例，實際需解析 splits 和 stats）
            # 為示範，暫時返回示例數據並註明需調整解析邏輯
            messagebox.showinfo("API 載入", "API 數據載入成功！（解析邏輯需根據實際 JSON 結構調整）")
            return self.load_sample_data()  # 回退到示例，實際替換為解析邏輯

        except requests.RequestException as e:
            messagebox.showerror("錯誤", f"無法載入真實數據: {str(e)}\n使用示例數據")
...
class MLBPlayerGUI:
    def __init__(self, root):
        ...
        # 顯示數據來源選擇對話框（在對話框完成後調用 create_widgets）
    ...
    def show_data_source_dialog(self):
        ...
        def use_real_data():
            ...
            self.create_widgets()  # 在數據載入後創建界面
    ...
    def load_real_data_with_progress(self):
        ...
        tk.Label(progress_dialog, text="正在載入MLB真實資料...", font=('Arial', 12)).pack(pady=20)
        ...
        def load_data():
            try:
                self.db.players_data = self.db.load_real_data()
                data_queue.put(("success", self.db.players_data))
            except Exception as e:
                data_queue.put(("error", str(e)))

        def check_queue():
            try:
                result_type, result = data_queue.get_nowait()
                if result_type == "success":
                    self.current_players = result.copy()
                    self.update_player_list()
                    self.update_statistics()
                    self.status_var.set("真實資料載入完成")
                else:
                    messagebox.showerror("錯誤", f"載入失敗: {result}")
                    self.load_sample_data()
    ...
    def load_sample_data(self):
        """載入示例數據"""
        self.db.players_data = self.db.load_sample_data()
        self.current_players = self.db.players_data.copy()
        self.update_player_list()
        self.update_statistics()
        self.status_var.set("示例資料載入完成")
    ...
    def create_batting_stats(self, parent):
        ...
        if not batters:
            tk.Label(hr_frame, text="無打者數據", font=('Arial', 10), anchor='w').pack(fill='x')
        else:
        ...
        if not batters:
            tk.Label(avg_frame, text="無打者數據", font=('Arial', 10), anchor='w').pack(fill='x')
        else:
        ...
        if not batters:
            tk.Label(rbi_frame, text="無打者數據", font=('Arial', 10), anchor='w').pack(fill='x')
        else:
    ...
    def create_pitching_stats(self, parent):
        ...
        if not pitchers:
            tk.Label(era_frame, text="無投手數據", font=('Arial', 10), anchor='w').pack(fill='x')
        else:
        ...
        if not pitchers:
            tk.Label(wins_frame, text="無投手數據", font=('Arial', 10), anchor='w').pack(fill='x')
        else:
        ...
        if not pitchers:
            tk.Label(k_frame, text="無投手數據", font=('Arial', 10), anchor='w').pack(fill='x')
        else:
    ...
    def create_salary_stats(self, parent):
        ...
        if not self.db.players_data:
            tk.Label(salary_rank_frame, text="無球員數據", font=('Arial', 10), anchor='w').pack(fill='x')
        else:
        ...
        if not self.db.players_data:
            tk.Label(summary_frame, text="無薪資數據", font=('Arial', 11), anchor='w').pack(fill='x')
        else:
...
平均年薪: ${avg_salary:.1f}M"""
    ...
    def show_player_detail(self):
        ...
        tk.Label(left_basic, text=f"守備位置: {player['position']}", 
                font=('Arial', 12), anchor='w').pack(fill='x', pady=2)
        tk.Label(left_basic, text=f"身高體重: {player.get('height', 'N/A')} / {player.get('weight', 'N/A')} lbs", 
                font=('Arial', 12), anchor='w').pack(fill='x', pady=2)
        ...
        tk.Label(right_basic, text=f"球衣號碼: #{player.get('jersey', 'N/A')}", 
                font=('Arial', 12), anchor='w').pack(fill='x', pady=2)
        tk.Label(right_basic, text=f"打擊/投球: {player.get('bats', 'N/A')}/{player.get('throws', 'N/A')}", 
                font=('Arial', 12), anchor='w').pack(fill='x', pady=2)
